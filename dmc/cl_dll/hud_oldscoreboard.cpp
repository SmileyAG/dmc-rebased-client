...
{
	m_pCvarOldScoreboard = CVAR_CREATE("cl_old_scoreboard", "1", FCVAR_ARCHIVE);
...
int CHudOldScoreboard::Draw(float fTime)
{
	...
	for (int iRow = 0; iRow < scoreboard->m_iRows; ++iRow)
	{
		...
		else if (scoreboard->m_iIsATeam[iRow] && gHUD.m_Teamplay)
		{
			...
			r = iTeamColors[team_info->teamnumber][0];
			g = iTeamColors[team_info->teamnumber][1];
			b = iTeamColors[team_info->teamnumber][2];
		...
		else
		{
			...
			r = iTeamColors[pl_info_extra->teamnumber][0];
			g = iTeamColors[pl_info_extra->teamnumber][1];
			b = iTeamColors[pl_info_extra->teamnumber][2];
			...
			// cut off the name if it'd overlap score
			while ( gHUD.GetHudStringWidth(szName) + nameoffset + NAME_RANGE_MIN + specoffset > KILLS_RANGE_MIN )
			{
				szName[strlen(szName) - 1] = '\0';
			}

			gHUD.DrawHudString(xpos + nameoffset, ypos, 0, szName, r, g, b);
